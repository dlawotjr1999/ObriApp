...
/*
 * KOPIS 동기화 오케스트레이션 — 오늘부터 SYNC_MONTHS_AHEAD개월 이내 클래식 공연을 페이지 단위로 가져와
 * 신규 저장·기존 갱신을 수행한다. 목록 응답만으로 Concert 저장에 필요한 필드가 전부 채워져(콩쿠르 크롤러와
 * 달리) 상세 조회가 없고, 빈 페이지를 만나면 마지막 페이지로 간주해 중단한다.
...
public class KopisSyncService {
    ...
    // 안전판 — 6개월치 클래식 공연이 5000건(50페이지 x 100건)을 넘길 일은 없다고 봄
    ...
    private int syncPages() {
        ...
        for (int page = 1; page <= HARD_CAP_PAGE; page++) {
            List<KopisPerformanceItem> items = fetchPageSafely(stdate, eddate, page);
            if (items == null || items.isEmpty()) {
                break; // 빈 페이지(또는 조회 실패) — 여기서 중단, 다음 스케줄 회차에서 처음부터 재시도
            }
    ...
    private List<KopisPerformanceItem> fetchPageSafely(String stdate, String eddate, int page) {
        try {
            Document document = client.fetchListDocument(stdate, eddate, page, ROWS_PER_PAGE);
            return KopisResponseParser.parse(document);
        } catch (KopisSyncException e) {
            log.warn("KOPIS 목록 페이지 조회 실패: page={}", page, e);
            return null;
        }
    }
